...
class Detector::SuggestedResourceTest < ActiveSupport::TestCase
  ...
  test 'fingerprints coerce characters to ASCII' do
    resource = {
      title: 'A wide range of characters',
      url: 'https://example.org',
      phrase: 'а а̀ а̂ а̄ ӓ б в г ґ д ђ ѓ е ѐ е̄ е̂ ё є ж з з́ ѕ и і ї ꙇ ѝ и̂ ӣ й ј к л љ м н њ о о̀ о̂ ō ӧ п р с с́ т ћ ќ у у̀ у̂ ӯ ў ӱ ф х ц ч џ ш щ ꙏ ъ ъ̀ ы ь ѣ э ю ю̀ я'
    }

    new_resource = Detector::SuggestedResource.create(resource)

    assert new_resource.fingerprint == 'a b ch d dj dz dzh e f g gh gj i ia ie io iu j k kh kj l lj m n nj o p r s sh shch t ts tsh u v y yi z zh'
  end
end
